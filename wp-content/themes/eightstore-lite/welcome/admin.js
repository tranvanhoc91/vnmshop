...
jQuery(document).ready(function($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = eightStoreWelcomeObject.ajaxurl;
    	...
    	var plNonce = newPlhref[newPlhref.length-1];
    	if(plNonce==''){
    		var plNonce = eightStoreWelcomeObject.admin_nonce;
    	}
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'eightstore_lite_plugin_installer',
				plugin: plugin,
			...
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(eightStoreWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'eightstore_lite_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'eightstore_lite_plugin_activation',
	   			plugin: plugin,
	   			nonce: eightStoreWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'eightstore_lite_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: eightStoreWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
